...
import { UploadCloud, CheckCircle, AlertCircle, Loader2, PieChart, FileText, List, Activity, Code, FolderSearch, ShieldAlert, Target, ShieldCheck, Filter, Bot, PlayCircle } from 'lucide-react';
import { reportClient, UploadResponse, AnalyzeResponse, ScanResponse, DetectGapsResponse, GenerateTestsResponse, RiskAnalysisResponse, DependencyAnalysisResponse, PredictionAnalysisResponse, ExecutiveDashboardResponse, GapFunctionData } from '../api/reportClient';
...
export default function UploadReport() {
  ...
  const [selectedRiskFunction, setSelectedRiskFunction] = useState<GapFunctionData | null>(null);
  ...
  const unhandledCount = useMemo(() => {
    ...
    return (gapResult.uncovered || 0) + (gapResult.partial || 0);
  }, [gapResult]);
                        ...
                        <div className="space-y-6">
                          {testResult.tests?.map((test, idx) => (
                            <div key={idx} className="bg-[#0d1117] border border-slate-700/50 rounded-xl overflow-hidden">
                              <div className="px-5 py-3 border-b border-slate-700/50 flex justify-between items-center bg-slate-800/20">
                                <div className="flex items-center space-x-3">
                                  <PlayCircle size={16} className="text-emerald-400" />
                                  <span className="text-sm font-medium text-slate-200">test_{test.function_name}.py</span>
                                </div>
                                <div className="flex items-center space-x-3">
                                  ...
                                  <span className="text-xs text-slate-500 flex items-center">
                                    <Bot size={12} className="mr-1" /> {test.model_used}
                                  </span>
                                </div>
                              </div>
